// gcc -I ../minilibx -L ../minilibx -lmlx -framework Appkit -framework OpenGL mving_circle_limit.c

/*
** jikang 의 코멘트: 원을 키를 눌러서 움직여보는 코드. 원이 윈도우 밖으로 넘어가지 않도록 한계를 넣어주었다.
**
*/
...
int				key_press(int keycode, t_vars *var)
{
	if (keycode == KEY_W) // UP
	{
		if (var->param.y - 100 >= 0)
			var->param.y -= 100;
	}
	if (keycode == KEY_A) // LEFT
	{
		if (var->param.x - 100 >= 0)
			var->param.x -= 100;
	}
	if (keycode == KEY_S) // DOWN
	{
		if (var->param.y + 100 + CIRCLE_HEIGHT <= WIN_HEIGHT)
			var->param.y += 100;
	}
	if (keycode == KEY_D) // RIGHT
	{
		if (var->param.x + 100 + CIRCLE_WIDTH <= WIN_WIDTH)
			var->param.x += 100;
	}
	if (keycode == KEY_F) // CHANG COLOR
